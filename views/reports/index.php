...
<script>
    window.addEventListener('load', async () => {
        ...
        let baseUrl = 'https://b24-oy9apg.bitrix24.com/rest/9/e3hbkx5cs7wy7r7r';
        let properties = [];
        const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=1036&select[0]=ufCrm5OfferingType&select[1]=ufCrm5PfEnable&select[2]=ufCrm5BayutEnable&select[3]=ufCrm5DubizzleEnable&select[4]=ufCrm5WebsiteEnable`);
        const data = await response.json();
        const totalOwners = data.total;

        for (let i = 0; i < Math.ceil(totalOwners / 50); i++) {
            const response = await fetch(`${baseUrl}/crm.item.list?entityTypeId=1036&select[0]=ufCrm5OfferingType&select[1]=ufCrm5PfEnable&select[2]=ufCrm5BayutEnable&select[3]=ufCrm5DubizzleEnable&select[4]=ufCrm5WebsiteEnable&start=${i * 50}`);
            const data = await response.json();
        ...
        properties.forEach(property => {
            if (property['ufCrm5OfferingType'] === 'RS' || property['offeringType'] === 'RR') {
                // Residential
                if (property['ufCrm5PfEnable']) residentialPropertyFinder++;
                if (property['ufCrm5BayutEnable']) residentialBayut++;
                if (property['ufCrm5DubizzleEnable']) residentialDubizzle++;
                if (property['ufCrm5WebsiteEnable']) residentialWebsite++;
                if (property['ufCrm5OfferingType'] === 'RS') {
                    residentialSale++;
                } else {
                    residentialRent++;
                }
            } else if (property['ufCrm5OfferingType'] === 'CS' || property['offeringType'] === 'CR') {
                // Commercial
                if (property['ufCrm5PfEnable']) commercialPropertyFinder++;
                if (property['ufCrm5BayutEnable']) commercialBayut++;
                if (property['ufCrm5DubizzleEnable']) commercialDubizzle++;
                if (property['ufCrm5WebsiteEnable']) commercialWebsite++;
                if (property['ufCrm5OfferingType'] === 'CS') {
                    commercialSale++;
                } else {
                    commercialRent++;
                }
