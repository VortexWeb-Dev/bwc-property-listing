...
<script>
    ...
    async function fetchAgents(page = 1) {
        ...
        const apiUrl = `${baseUrl}/crm.item.list?entityTypeId=${entityTypeId}&order[id]=desc&select[0]=id&select[1]=ufCrm7AgentName&select[2]=ufCrm7AgentEmail&select[3]=ufCrm7AgentMobile&select[4]=ufCrm7AgentLicense&select[5]=ufCrm7AgentPhoto&start=${(page - 1) * pageSize}`;
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 flex items-center space-x-2">
                        <img 
                            ...
                            src="${agent.ufCrm7AgentPhoto || './assets/images/agent-placeholder.webp'}" 
                            alt="${agent.ufCrm7AgentName || 'Agent'}" />
                        <span>${agent.ufCrm7AgentName || 'Agent'}</span>
                    </td>

                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm7AgentEmail.toLowerCase() || ''}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm7AgentMobile || ''}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${agent.ufCrm7AgentLicense || ''}</td>
    ...
    async function deleteAgent(agentId) {
        const baseUrl = 'https://b24-oy9apg.bitrix24.com/rest/9/e3hbkx5cs7wy7r7r';
        const apiUrl = `${baseUrl}/crm.item.delete?entityTypeId=1040&id=${agentId}`;
